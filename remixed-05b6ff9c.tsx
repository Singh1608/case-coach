...
const SYSTEM_PROMPT = (caseData, style) => {
...
export default function CaseCoach() {
  ...
  const speak = useCallback((text) => {
    ...
    const preferred = voices.find(v => v.name.includes("Google UK English Male") || v.name.includes("Daniel") || v.name.includes("Alex") || v.name.includes("en-GB"));
  ...
  const callAPI = async (msgs, caseData) => {
    const res = await fetch("https://api.anthropic.com/v1/messages", {
      method: "POST",
      headers: { "Content-Type": "application/json", "anthropic-version": "2023-06-01", "anthropic-dangerous-direct-browser-access": "true" },
      body: JSON.stringify({ model: "claude-sonnet-4-5", max_tokens: 1000, system: SYSTEM_PROMPT(caseData, caseData.style), messages: msgs }),
    });
  ...
  const sendMessage = async (overrideInput) => {
    ...
    try {
      ...
      speak(text);
    } catch (err) { setMessages([...updated, { role: "assistant", content: "Error: " + err.message }]); }
    setLoading(false);
    setTimeout(() => inputRef.current?.focus(), 100);
  };
    ...
    <div style={{ height: "100vh", display: "flex", flexDirection: "column", background: "linear-gradient(180deg, #0f0c29 0%, #1a1a2e 100%)", fontFamily: "'Segoe UI', system-ui, sans-serif", color: "#f1f5f9" }}>
      ...
      <div style={{ padding: "14px 20px", borderBottom: "1px solid rgba(255,255,255,0.08)", display: "flex", alignItems: "center", gap: 14, background: "rgba(15,12,41,0.8)", backdropFilter: "blur(12px)", flexShrink: 0 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", marginTop: 2 }}>{selectedCase?.type} · {selectedCase?.style} · {selectedCase?.difficulty}</div>
        </div>
